...
import { addExpense, getTodayExpenses } from '../utils/firestoreHelpers';
...
function Expenses() {
  ...
  const handleAddExpense = async (index) => {
    ...
    if (!auth.currentUser) {
      setMessage({ text: 'Please log in to save expenses', type: 'error' });
      return;
    }
    ...
    try {
      ...
      updated[index].expenses.push({ amount, remark: formData.remark });
  ...
  const handleAddCategory = () => {
    ...
    setCategories([...categories, { name: newCategory.trim(), expenses: [] }]);
    setNewCategory('');
